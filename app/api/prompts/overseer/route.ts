...
import { getOverseerPrompt, updateOverseerPrompt } from "@/lib/data/prompts";
...
export async function GET(request: Request) {
  ...
  const route = "/api/prompts/overseer";
  const promptKey = "overseer";
  const startedAt = Date.now();

  try {
    const session = await requireAuthenticatedSession(request, requestId);
    const prompt = await getOverseerPrompt();
    const resolvedPrompt = prompt?.content ?? DEFAULT_OVERSEER_ROLE_PROMPT;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const saved = await updateOverseerPrompt(
      promptText,
      session.userId,
      session.requestId,
